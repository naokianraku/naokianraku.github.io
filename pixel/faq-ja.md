...
**Q. Premium はいくらですか？**
Premium は自動更新のサブスクリプションです。提供中のプラン・お住まいの地域の
価格・無料期間の有無は、Google Play の購入画面に表示されます。
...
施設マップ）も無料です。Health Connect を連携した場合の**睡眠スコア**と**安静時
心拍の参照ライン**も無料です。
...
**Q. 自動更新はどうなっていますか？**
各請求期間の終わりに自動で更新されます。次の更新日より前に Google Play で
キャンセルすれば、それ以降は請求されません。購入時に無料期間が表示されていた
場合も同じで、無料期間の終了までにキャンセルすれば課金されません。

**Q. キャンセル方法は？**
Google Play ストアを開く → プロフィール → 「**お支払いと定期購入**」 →
「**定期購入**」 → 本アプリの Premium → 「定期購入を解約」。キャンセル後も、
...
Google アカウント → セキュリティ → 「サードパーティ製のアプリとサービスとの接続」
から本アプリの接続を削除すれば、いつでも**取り消せます**（アプリ内に接続を解除
する設定はありません）。
...
して表示したり、設定の「**セッションを Health Connect に書き出し**」で
セッションを Health Connect に運動 + 心拍の記録として**書き出して**他のアプリで
使えるようにできます（書き出しは手動で、自動では行いません）。Health Connect
の連携は完全に任意で、連携しなければ従来どおり、ヘルスデータ連携なしで動作します。
...
設定 → プライバシー → 広告）に従います。本アプリは現在、日本国内のみで配信して
います。Premium では広告が表示されません。詳細はプライバシーポリシー参照。
...
いいえ。**計測（タイマーと心拍）はウォッチアプリで行うため、Pixel Watch（または
他の Wear OS スマートウォッチ）が必要です。** スマホアプリは、ウォッチから届いた
セッションの閲覧と分析に使います。
...
**Q. セッション中に「防水ロック（水ロック）」を使えますか？**
**使わないことをおすすめします。** Wear OS の水ロックは**クラウンを回して解除する**
仕様のため、ロック中はフェーズ移行・一時停止のクラウン操作もできなくなります。
本アプリは**計測中は既に画面タッチを無視**する（濡れた手・水しぶきの誤操作防止）ので、
水ロックは不要です。セッション中は水ロックをかけず、そのままクラウンで操作してください。
...
施設を 1 枚の地図に表示する**施設マップ**が表示されます。いずれも無料で、アプリ
...
ベストセッション TOP 10、訪問頻度ヒートマップ、施設マップ）。**Premium：**
...
を選べます。保存した画像は端末自身の写真（Pictures/MaxRecoveryTimer）に入り、どこにも
アップロードされません。
...
両デバイスで MaxRecovery を開いて近くに置き、Bluetooth（または Wi-Fi）を確認、
...
**Q. アプリを英語表示にできますか？**
スマホアプリはできます。スマホアプリは日本語と英語に対応しており、設定を開いて
「**英語表示（強制）**」トグルをオンにすると、システム言語に関係なく英語 UI に
固定されます。ウォッチアプリは現在、日本語表示のみです（このトグルはウォッチには
反映されません）。
