...
- Heart-rate readings come from the Pixel Watch optical sensor and may be
  inaccurate, especially in the hot, humid, wet conditions of a sauna. The watch
  is not rated for such environments.
...
- Refund requests are handled via your Google Play order history
  (play.google.com), subject to Google's policies.
...
- 「ととのい度」スコア、心拍数値、チャート、その他表示される数値はすべて
  ...
  判断に使用してはなりません。
- 心拍数値は Pixel Watch の光学式センサーによるもので、特にサウナの高温・
  多湿・濡れた環境では誤差を含むことがあります。本ウォッチはこうした環境での
  使用を想定していません。
...
- 返金のご依頼は Google Play の注文履歴（play.google.com）から、Google の
  ポリシーに従って行えます。
