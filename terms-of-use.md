...
By downloading or using MaxRecovery Timer ("the App"), you agree to these Terms of
...
- The "Afterglow Score", heart-rate values, charts, and any other figures shown
...
- Plans: monthly and yearly auto-renewing subscriptions. Pricing is shown in the
  App and on the App Store and may vary by region.
- A free introductory trial period may be offered. If you do not cancel before
  the trial ends, the subscription auto-renews at the stated price.
- Subscriptions renew automatically unless cancelled at least 24 hours before
  the end of the current period.
- Purchases and renewals are processed by Apple. Manage or cancel your
  subscription in your Apple Account settings.
- iCloud sync is available to all users at no charge.

### 4. Acceptable use

You agree not to misuse the App, including attempting to reverse-engineer,
...
damages arising from the use of, or inability to use, the App — including but
not limited to health-related outcomes, data loss, or service interruptions.
Nothing in these Terms limits liability that cannot be limited by law, and
...
MaxRecovery Timer（以下「本アプリ」）をダウンロードまたは利用することにより、
利用者は本利用規約に同意したものとみなされます。同意いただけない場合は本
アプリを利用しないでください。

### 2. 健康・安全に関する免責 — 重要

**本アプリは医療機器ではなく、医学的助言を提供するものではありません。**
...
- プラン: 月額および年額の自動更新サブスクリプション。価格はアプリ内および
  App Store に表示され、地域により異なる場合があります。
- 無料の試用期間が提供される場合があります。試用期間終了前に解約しない場合、
  表示された価格で自動的に更新されます。
- サブスクリプションは、現在の期間終了の 24 時間以上前に解約しない限り自動
  更新されます。
- 購入および更新は Apple が処理します。サブスクリプションの管理・解約は
  Apple アカウントの設定から行えます。
- iCloud 同期はすべての利用者が追加料金なしで利用できます。
...
利用不能から生じるいかなる損害（健康に関する結果、データ損失、サービスの
中断を含むがこれらに限られない）についても責任を負いません。本規約は、
